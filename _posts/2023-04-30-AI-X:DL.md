...
>[**설명영상**](https://youtu.be/YtBlHGunxdk)
# 멤버 구성
...
[![DL vs. ML][DLvs.ML]{: .align-center}{: width="60%" height="60%"}]([DLvs.ML링크출처](https://medium.com/@sayondutta/nuts-and-bolts-of-applying-deep-learning-by-andrew-ng-89e1cab8b602))
...
![feature_importance](../_images/AI-X:DL/feature_importance.png){: .align-center}{: width="60%" height="60%"}
 
교습비에 주요하게 영향을 미치는 요인이 무엇인지 알아보기 위해, 비교적 성능이 좋게 나온 decision tree 모델에서 feature 들의 중요도를 평가했고, 수치가 유의미했던 상위 6개의 피처들의 중요도를 정리해보았다. 이를 통해 앞서 토큰화를 통해 데이터 프로세싱을 했던 교습과목(반) 이 학원의 교습비에 영향을 가장 많이 미치는 요인이라는 것을 알 수 있었고, 그 다음으로 학원의 강사 수, 1km 반경 내에 있는 주변 학교들의 학생 수, 주변 공립(사립) 학교 개수가 유의미하게 학원비 예측에 영향을 미친다는 것을 알 수 있었다.
