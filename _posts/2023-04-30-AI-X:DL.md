...
# 학교 급별 수 추출
df_aca_loc['ele_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('학교급', ['초등학교'], x).shape[0])
df_aca_loc['mid_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('학교급', ['중학교'], x).shape[0])
df_aca_loc['high_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('학교급', ['고등학교'], x).shape[0])
df_aca_loc['sch_count'] = df_aca_loc.ele_count + df_aca_loc.mid_count + df_aca_loc.high_count

# 학교 설립 유형 별 수 추출
df_aca_loc['pub_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('설립', ['공립','국립'], x).shape[0])
df_aca_loc['priv_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('설립', ['사립'], x).shape[0])

# 학교 급별 학생 수 추출
df_aca_loc['ele_std_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('학교급', ['초등학교'], x)['학생수(계)'].sum())
df_aca_loc['mid_std_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('학교급', ['중학교'], x)['학생수(계)'].sum())
df_aca_loc['high_std_count'] = df_aca_loc['near_sch'].apply(lambda x: school_sort_df('학교급', ['고등학교'], x)['학생수(계)'].sum())
df_aca_loc['sch_count'] = df_aca_loc.ele_std_count + df_aca_loc.mid_std_count + df_aca_loc.high_std_count
```

## 3. '교습과목(반)'열 데이터 프로세싱

교습과목(반) 열에는 해당 학원에서 진행하는 교육 프로그램에 관한 세부적인 내용이 담겨있다. 하지만 학원별로 과정을 부르는 명칭이 다 다를 뿐만 아니라, 같은 학원에서 진행하는 내용이더라도 그 표현을 다르게 한 경우가 존재했다. 예를 들어 고등학교 2학년 학생을 대상으로 진행하는 국어 내신 대비 교습과정의 경우 한 학원에서는 '고2국어내신'이라고 표현하는 반면, 다른 학원에서는 '국(고등2)'라고 표현하는 등 다양했다. 해당 열은 처리하기 복잡하지만 내용이 많고 교습비를 예측하는 데 가장 큰 기여를 할 것으로 보여 서울대학교에서 연구한 한국어 토큰화 알고리즘 꼬꼬마(Kkma)를 활용해 분석하였다.

아래는 토큰화 세부 코드이다.
```python
from konlpy.tag import Kkma # 패키지 임포트
kkma = Kkma()
...
그 후, Kkma 태깅 규칙에 따라 유효한 태깅만 리스트로 반환해 데이터에 추가하였다.
```python
import ast

tagging_list = ['NNG', 'NNP']
...
## 4.Visualization(ongoing)
(ongiong) - 지호님 파트, 여건상 어려우면 빼기

## 5. Algorithms or Methods
(ongoing) - 소미님 파트
### 5-(1). MLP
...
# 데이터 불러오기
data = pd.read_csv('../../data/model_fitting/모형적용데이터.csv', index_col=0)
data.drop(columns=['교습과목(반)', 'tk_text'], inplace=True)
X = data.drop(columns='교습비')
y = data.교습비
```

수치형 데이터는 ```MinMaxScaler()```를 사용하여 표준화하였고, 범주형 데이터는 원핫인코딩을 통해 더미변수로 확장하였다.
```python
# 수치형 데이터 처리: 표준화
...
```
모형의 조율모수를 조정하면서 최적의 파라미터를 찾고자 했다.
```python
# 하이퍼파라미터 튜닝 및 모델 학습
epochs_list = [50, 100]
...
![머신러닝 알고리즘 RMSE 비교](./../_images/AI-X:DL/algorithm_output.png)
...
\[참고문헌\]
- [BallTree와 거리 계산](https://scikit-learn.org/stable/modules/generated/sklearn.neighbors.BallTree.html)
- [네이버 지도 API](https://api.ncloud-docs.com/docs/ai-naver-mapsgeocoding)
